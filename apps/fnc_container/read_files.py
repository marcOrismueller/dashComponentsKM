...
def update_data():
    # Get the folder path & the last time we readed (Timestamp)
    metadata = crud_op_db.get_printer_folder_path()
    # Read the new files only
    card_header_list, card_body_list, newLastFileReaded = read_files(
        folderPath=metadata[0], 
        lastFileReaded=float(metadata[1])
    )
    if len(card_header_list) == 0:
        return False 
    ...
    crud_op_db.update_lastFileTs(str(newLastFileReaded))
    return True